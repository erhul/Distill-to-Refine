...
def copy_target_simp(args):
    ...
    while iter_num < max_iter:
        ...
        # if args.ema < 1.0 and iter_num > 0 and iter_num % (3 * len(dset_loaders["target"])) == 0:
        #     model.eval()
        #     start_test = True
        #     with torch.no_grad():
        #         iter_test = iter(dset_loaders["target_te"])
        ...
        if epoch >= 30 and iter_num % len(dset_loaders["target"]) == 0:
            model.eval()
        ...
        if epoch < 30:
            optimizer, decay = lr_scheduler(optimizer, iter_num=iter_num, max_iter=50 * len(dset_loaders["target"]), power=1.5)
            # optimizer, decay = lr_scheduler(optimizer, iter_num=iter_num, max_iter=30 * len(dset_loaders["target"]), power=1.5)
        else:
        ...
        if iter_num % (3 * len(dset_loaders["target"])) == 0 or iter_num == max_iter:
            model.eval()
            if args.dset == 'VISDA-C':
