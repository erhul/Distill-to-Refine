...
def copy_target_simp(args):
    ...
    with torch.no_grad():
        ...
        print(str)
        if args.wandb:
            # wandb.log({'mem_acc': accuracy * 100}, step=0)
            wandb.log({'test_acc': accuracy * 100}, step=0)
    ...
    while iter_num < max_iter:
        ...
        #         print(str)
        #         # if args.wandb:
        #         #     wandb.log({'mem_acc': accuracy * 100}, step=e)
        #
        #     model.train()
        ...
        lr_scheduler(optimizer, iter_num=iter_num, max_iter=50 * len(dset_loaders["target"]), power=1.5)
        ...
        if pre_data != None:
            ...
            kl_loss = nn.KLDivLoss(reduction='batchmean')(outputs_target.log(), outputs_target_by_source)
            optimizer.zero_grad()
        ...
        if iter_num % (3 * len(dset_loaders["target"])) == 0 or iter_num == max_iter:
            ...
            if args.wandb:
                wandb.log({'test_acc': acc_s_te}, step=e+1)
...
if __name__ == "__main__":
    ...
    if args.wandb:
        wandb.init(project="Distill to Refine for Black-Box Domain Adaptation", name="AC")
