...
def cal_acc02(loader, model, flag=False):
    start_test = True
    with torch.no_grad():
        iter_test = iter(loader)
        for i in range(len(loader)):
...
def test_target_simp(args):
    ...
    if args.dset == 'VISDA-C':
        acc, acc_list, _ = cal_acc(dset_loaders['test'], netF, None, netC, True)
...
# @torch.no_grad()
def eval_and_label_dataset(epoch, model, target_te_loader):
...
def contrastive_loss02(logits_ins):
    # labels: positive key indicators
    labels_ins = torch.zeros(logits_ins.shape[0], dtype=torch.long).cuda()
    loss = F.cross_entropy(logits_ins, labels_ins)
...
def copy_target_simp(args):
    ...
    model = network.Resnet(args).cuda()
    # momentum_model = network.Resnet(args).cuda()
    ...
    # moco_model = AdaMoCo(src_model=model, momentum_model=momentum_model, features_length=args.bottleneck,
    #                      num_classes=args.class_num, dataset_length=len(dset_loaders["target"].dataset),
    ...
    while iter_num < max_iter:
        ...
        # if args.ema < 1.0 and iter_num > 0 and iter_num % len(dset_loaders["target"]) == 0:
        #     model.eval()
        #     start_test = True
        #     with torch.no_grad():
        #         iter_test = iter(dset_loaders["target_te"])
        ...
        #         accuracy = torch.sum(torch.squeeze(predict).float() == all_label).item() / float(all_label.size()[0])
        #         str = "new update menP accuracy = {:.2f}%".format(accuracy*100)
        #         args.out_file.write(str + '\n')
        #         args.out_file.flush()
        #         print(str)
        #
        ...
        if iter_num >= dist_iter and iter_num % len(dset_loaders["target"]) == 0:
            model.eval()
            acc, banks, _, pred_labels, pred_probs = eval_and_label_dataset(e, model, dset_loaders["target_te"])
            model.train()
        try:
        ...
        if iter_num <= dist_iter:
            optimizer, decay = lr_scheduler(optimizer, iter_num=iter_num, max_iter=10 * len(dset_loaders["target"]), power=1.5)
        else:
        ...
        if pre_data != None:
            if iter_num <= dist_iter:
                _, outputs_target = model(inputs_target[:, 0, ...])
                _, out_pre = model(pre_data[:, 1, ...])
                ...
                optimizer.zero_grad()
                # decay_dml = 1 if epoch >= 3 else epoch / 3
                ...
                if args.mix > 0:
                    ...
                    _, outputs_target_m = model(mixed_input)
                ...
                outputs_target = None
                _, logits_q = model(inputs_target_aug1)
                _, logits_k = model(inputs_target_aug2)
                current_batch_outputs = torch.cat((logits_q, logits_k), dim=0)
                current_batch_outputs = torch.nn.Softmax(dim=1)(current_batch_outputs)

                with torch.no_grad():
                    # pseudo_labels_w, probs_w, _, _ = refine_predictions(feats_target, banks)
                ...
                if args.mix > 0:
                    ...
                    update_batch_stats(model, False)
                    feats_target_m, outputs_target_m = model(mixed_input)
                    update_batch_stats(model, True)
                    outputs_target_m = torch.nn.Softmax(dim=1)(outputs_target_m)
                    mix_loss = args.mix * nn.KLDivLoss(reduction='batchmean')(outputs_target_m.log(), mixed_output)
                    mix_loss.backward()
        ...
        if iter_num % len(dset_loaders["target"]) == 0 or iter_num == max_iter:
            model.eval()
            if args.dset == 'VISDA-C':
                acc_s_te, acc_list, mean_ent = cal_acc02(dset_loaders['test'], model, True)
                log_str = 'Task: {}, Iter:{}/{}; Accuracy = {:.2f}%, Ent = {:.4f}'.format(args.name, iter_num, max_iter, acc_s_te, mean_ent) \
                          + '\n' + acc_list
            else:
                acc_s_te, mean_ent = cal_acc02(dset_loaders['test'], model, False)
                log_str = 'Task: {}, Iter:{}/{}; Accuracy = {:.2f}%, Ent = {:.4f}'.format(args.name, iter_num, max_iter, acc_s_te, mean_ent)
        ...
        if iter_num / len(dset_loaders["target"]) == 1:
            torch.save(model.netF.state_dict(), osp.join(args.output_dir_1, "source_F.pt"))
            torch.save(model.netB.state_dict(), osp.join(args.output_dir_1, "source_B.pt"))
            torch.save(model.netC.state_dict(), osp.join(args.output_dir_1, "source_C.pt"))
        if iter_num / len(dset_loaders["target"]) == 10:
            torch.save(model.netF.state_dict(), osp.join(args.output_dir_10, "source_F.pt"))
            torch.save(model.netB.state_dict(), osp.join(args.output_dir_10, "source_B.pt"))
            torch.save(model.netC.state_dict(), osp.join(args.output_dir_10, "source_C.pt"))

    torch.save(model.netF.state_dict(), osp.join(args.output_dir, "source_F.pt"))
    torch.save(model.netB.state_dict(), osp.join(args.output_dir, "source_B.pt"))
    torch.save(model.netC.state_dict(), osp.join(args.output_dir, "source_C.pt"))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dist_epoch', type=int, default=10, help="max iterations")
    ...
    if args.distill:
        for i in range(len(names)):
            ...
            args.output_dir_1 = osp.join(args.output, args.net_src + '_' + args.net, str(args.seed), args.da, args.dset,
                                         names[args.s][0].upper() + names[args.t][0].upper(), '1')
            args.output_dir_10 = osp.join(args.output, args.net_src + '_' + args.net, str(args.seed), args.da, args.dset,
            ...
            if not osp.exists(args.output_dir_1):
                os.system('mkdir -p ' + args.output_dir_1)
            if not osp.exists(args.output_dir_1):
                os.makedirs(args.output_dir_1)

            if not osp.exists(args.output_dir_10):
                os.system('mkdir -p ' + args.output_dir_10)
            if not osp.exists(args.output_dir_10):
                os.makedirs(args.output_dir_10)
