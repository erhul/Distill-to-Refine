...
def knn_predict_internal(feature, feature_bank, feature_labels, classes, knn_k, knn_t):
    ...
    # [B, K]
    a = feature_labels.expand(feature.size(0), -1)
    ...
    # weighted score ---> [B, C]
    b = one_hot_label.view(feature.size(0), -1, classes)
    c = one_hot_label.view(feature.size(0), -1, classes) * sim_weight.unsqueeze(dim=-1)
