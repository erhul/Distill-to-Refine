...
import wandb
# os.environ["WANDB_API_KEY"] = 'a1666d1aa599f1714c5170c5a5684758877a6da8'
...
def copy_target_simp(args):
    ...
    while iter_num < max_iter:
        ...
        if iter_num <= dist_iter:
            ...
            # _, decay = lr_scheduler(optimizer, iter_num=iter_num, max_iter=30 * len(dset_loaders["target"]), power=1.5)
        else:
        ...
        if pre_data != None:
            ...
            if iter_num <= dist_iter:
                ...
                # decay_dml = 1 if epoch >= 3 else 0
                # decay_dml = 1 if epoch >= 3 else (epoch / 3) ** 1.5
                ...
                delay_mi = 0 if iter_num / interval_iter > 1 else 1
                ...
                mi_loss = delay_mi * mi_loss

                total_loss = kl_loss + dml_loss + mi_loss
                total_loss.backward()

                delay_mix = 0 if iter_num / interval_iter > 1 else 1
                if args.mix > 0:
                    alpha = 0.3
                    ...
                    mix_loss = delay_mix * nn.KLDivLoss(reduction='batchmean')(outputs_target_m.log(), mixed_output)
                    mix_loss.backward()
                optimizer.step()
            else:
                outputs_target = None
...
if __name__ == "__main__":
    ...
    if args.distill:
        ...
        args.output_dir = osp.join(args.output, args.net_src + '_' + args.net, str(args.seed), args.da, args.dset,
                                   names[args.s][0].upper() + names[args.t][0].upper(), 'ablation04')
